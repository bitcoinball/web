...
function startPowerballAnimation() {
  ...
  function animate() {
    ...
    miniParticles = miniParticles.filter(particle => {
      if (particle.collisionCount >= 1 && numberBalls.length < 6) {
        // 创建数字球
    ...
    if (numberBalls.length === 6 && miniParticles.length === 0 &&
      ...
      const startX = (powerballCanvas.width - totalWidth) / 2;
      const spacing = totalWidth / (numberBalls.length - 1);
      const bottomY = powerballCanvas.height - 50;
      ...
      numberBalls.forEach((ball, index) => {
        ...
        ball.y = bottomY - ball.radius; // Position above bottom
